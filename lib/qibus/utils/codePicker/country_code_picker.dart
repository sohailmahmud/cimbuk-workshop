...
import 'package:prokit_flutter/qibus/utils/codePicker/selection_dialog.dart';
...
//                : Container(),
            Flexible(
              // fit: widget.alignLeft ? FlexFit.tight : FlexFit.loose,
              child: text(selectedItem.toCountryCodeString(), textColor: qIBus_colorPrimary, fontSize: textSizeMedium),
            ),
          ],
        ),
      );
  ...
  initState() {
    ...
    favoriteElements = elements.where((e) => widget.favorite.firstWhere((f) => e.code == f.toUpperCase() || e.dialCode == f.toString(), orElse: () => null) != null).toList();
    super.initState();
  }
